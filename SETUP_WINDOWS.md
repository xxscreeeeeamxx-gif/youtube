...
### なぜ素材をGitHubに置けないのか（2026-09-18 調査）

「USBで運ぶのが面倒だからGitに入れられないか」は**できない**。容量ではなく
ライセンスの問題で、3種とも別々の理由で塞がっている。
**このリポジトリは公開設定**であることにも注意。

| 素材 | 出典 | 判断 |
|---|---|---|
| `assets/bgm/` | OpenTracks（旧DOVA-SYNDROME） | **不可**。「営利、非営利を問わず、音源を配布、または販売すること」が禁止で、注釈に「オープン・クローズドを問わずインターネット上での公開」とある。**非公開リポジトリでもだめ** |
| `assets/se/` | 効果音ラボ | **不可**。禁止事項に「再配布禁止」、例として「効果音を販売もしくは配布」 |
| `assets/characters/` | 坂本アヒル氏の立ち絵 | **不可**。二次配布禁止 |
| `client_secret.json` | Google Cloud | **不可**。認証情報。公開リポジトリに置けば第三者にチャンネルの投稿権限が渡る |

**自分の別のPCに自分のコピーを移すのは配布ではない**ので、USBメモリや
自分のクラウドドライブ（Google Drive / OneDrive 等の非共有フォルダ）は問題ない。
運ぶのは80MBなのでどちらでも数分で済む。

**将来このリポジトリで作業するとき、良かれと思ってこれらをコミットしないこと。**
