...
**饒舌に畳みかける一人語り＋視聴者への強めの煽り**:
- 短文をリズムよく重ねる。「で、満充電。これはもう、朝8時の通勤電車です」
- 視聴者に直接踏み込む: 「〜していませんか」「心当たり、ありますよね」
  「ええ、あなたです」「目をそらさない」「やらない理由、ありますか」— 各章に2回程度、遠慮しない
- 上から目線にせずユーモアで中和（煽った直後に笑いか救いを置く）。かぎ括弧「」は使わない
- **誰でも一発で分かる表現だけ使う**: 強引な比喩（妖精さん→満員電車のような飛躍）、
  内輪ノリのスラング（科学で潰す・真面目系ポンコツ・健気じゃない？等）は禁止。
  聞いて2秒で意味が取れない言い回しは書き直す。機能の説明は擬人化より動作をそのまま言う
- 音声は各キャラのデフォルトスタイル固定（感情でスタイルを切り替えない。聞き取りやすさ優先）。話速は1.15
- 事実に自信がない数字・研究は「〜と言われています」「〜という報告があります」
- 難読語は `[表示|よみ]` 記法。恒久的な誤読は `ytf dict add <表記> <よみ>`
- 章構成: フック（煽り＋テーマ提示・short: true）→ 本編2〜3章 → 実用 → 締め。
  **まとめ・振り返りの章は作らない**。締めの章は `title: ""`（トランジションも見出しも出ない）にして、
  行動を促す一言 → 逆説の一言 → 登録呼びかけ、の3〜4カットで一気に終わる
- 仕組み・数字・手順の解説はPillowで自作アニメを作って `video:` で全画面埋め込みする
  （例: `scripts/gen_battery_anim.py`、`scripts/gen_infographics.py` = ゲージ/グラフ/トグル/チェックリスト）。
  尺は timing.json のカット実測に合わせ、ループしないよう1秒前後余らせる。
  **背景がゆっくり動くだけの時間を作らない**: 各章に最低1つは動く図解を置き、
  数十秒ごとに画面に変化（アニメ・スライド・stat・テロップ）が起きるようにする
...
| `ytf op` | 5秒のチャンネルOPを生成（導入→OP→解説 の位置に自動挿入される） |
