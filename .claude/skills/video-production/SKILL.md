...
- **制作指示をセリフに混ぜない**: 「ここが動画の核心」のようなメタ発言は
  ユーザーからの指示であってナレーションではない。台本には内容だけを書く
- **前振りで期待を煽らない**: 「ここが今日いちばん面白いところ」「ここからがかっこいいところ」
  のような前置きは書かず、内容そのもので語る
- カットに `duet_with: <キャラ>` を付けると同じセリフを2人同時に言う（挨拶の斉唱など）。
  音声は2人分ミックス・立ち絵は両方明るく表示。動画の締めは
  「ご視聴、ありがとうございました！」を2人のデュエットで言って終わる
- BGMはシーンの `bgm:` で章ごと・話題ごとに切り替える（assets/bgm/の名前。
  未指定は前シーンから継続、変わり目は自動フェード）。曲の使い分けは assets/bgm/SOURCES.md 参照。
  タイトル無しシーン（`title: ""`）に分ければ、章カードを出さずにBGMだけ切り替えられる
- 掛け合い形式の口調: つむぎの語尾は 〜よ／〜だよ／〜ね 系。ずんだもんは 〜なのだ／〜のだ（一人称ボク）。
  つむぎがずんだもんを呼ぶときは必ず**ずんだもん**（キミは使わない）。視聴者への呼びかけは「あなた」
- **1文を長めに**: 短文をブツブツ切らず、接続助詞（〜から、〜けど、〜して）で自然につなぐ。
  体言止めの連発や「つまり、燃えにくい。」のような断片は禁止
- **多読みの漢字は読みタグ必須**: 空(から/そら)・辛い(からい/つらい)・行った(いった/おこなった)等は
  `[空|から]` のように読みを固定するか、別の言葉（空っぽ等）に言い換える。
  whisper QCは同音異義を検出できないので、心配な語は timing.json の moras で読みを直接確認する
- 立ち絵: 出演キャラ全員に `sprite_dir` があるプロジェクトだけ自動表示（青山単独回は従来レイアウト）。
  台本の `emotion:` は立ち絵の表情にだけ効く（音声スタイルは固定）。素材は二次配布禁止のため
  gitに入れない（assets/characters/SOURCES.md 参照）。PSDからの書き出しは
  `scripts/export_zundamon_sprites.py` / `export_tsumugi_sprites.py`。
  全画面図解を作るときは画面下部の左右（立ち絵の定位置）に重要な文字を置かない
