...
| 直したいこと | 場所 |
|---|---|
| 手動の微修正全般 | `ytf edit <slug>` — ブラウザの編集画面。セリフ・読み・テロップ・スライドを本番同一のプレビューを見ながら調整し、そのカットだけ試聴もできる |
| 素材を探して入れる | `ytf edit` の「🔍 素材を探す」→ 検索語（英語推奨）→ サムネをクリックで「背景に」/「カードに」挿入。DL・パス設定・ライセンス記録まで自動（media/credits.txt）。動画はPEXELS_API_KEY設定時のみ |
