...
- 1セリフは60文字以内。長い説明は複数セリフに分割してテンポを作る（1カット＝1テロップとして画面に出る）
- 3〜5セリフごとに、驚き・意外な事実・たとえ話のどれかを入れる
- 立ち絵は出さない構成。情報は slide（図解カード）とテロップで見せる。重要な数字・要点は必ず slide に出す（bullets は1枚3行まで、各行20文字以内）
